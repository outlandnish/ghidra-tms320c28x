...
4d. `MarkComponentRegistry.java` — the other half of materialization. Materializing restores the *bytes* of the RAM dispatch table; this emits the *references*, so the call graph finally shows the indirect edges. Finds the table structurally, reads each dispatcher's descriptor field offset out of the instruction stream (the immediate before the `LCR *XARn`), creates functions at proven call targets, and recovers a dispatcher's own function when nothing calls it either. Where a site has no table behind it, it propagates the register the call dispatches through back to one concrete base + field offset and emits that single edge (`-Dc28x.reg.noBaseResolve` to skip) — straight-line only, exact function entry only, and anything a runtime index touches is reported rather than guessed at. Idempotent. Also roots interrupt handlers from the PIE vector table's flash initializer and names them from `data/device_profiles/` (`FUN_00095de5` → `CANB0_INT`); that pass runs even with no dispatch registry, since its evidence is in flash. See [data/device_profiles/README.md](data/device_profiles/README.md).
