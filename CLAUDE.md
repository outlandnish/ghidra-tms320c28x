...
1. `SeedFunctions.java` — seeds from LCR/LC/FFC call targets + prologue patterns (entropy/code-likeness filtered), plus the **C-runtime entry** `_c_int00`, which nothing calls and no prologue marks (see below).
...
4c. `EmulateStartup.java` — the general alternative to 4/4b: **runs the image's own `_c_int00`** under the emulator and keeps the RAM it writes, so it covers whatever mechanism the image uses — including the `.cinit` walk TI inlines into `_c_int00`, which neither materializer implements. Dry run by default (`apply` to write). It is also the sharpest ISA test in the tree: replaying real startup found two flag holes and the `*XAR7` repeat shadow that decode parity cannot see.
4d. `MarkComponentRegistry.java` — the other half of materialization. Materializing restores the *bytes* of the RAM dispatch table; this emits the *references*, so the call graph finally shows the indirect edges. Finds the table structurally, reads each dispatcher's descriptor field offset out of the instruction stream (the immediate before the `LCR *XARn`), creates functions at proven call targets, and recovers a dispatcher's own function when nothing calls it either. Where a site has no discovered table behind it, two passes work from the call site: **base resolution** propagates the register the call dispatches through back to one concrete base + field offset and emits that single edge (`-Dc28x.reg.noBaseResolve`); **strided tables** takes the sites it refuses — a runtime-indexed walk — and reads base, stride and field offset out of the dispatcher, then walks records until one holds a word that is neither null nor code (`-Dc28x.reg.noStrided`); both operand orders of the index add are recognized, and a base landing inside an already-accepted table is treated as a cursor into it. Both are straight-line and require an exact function entry. The call-site and rooting passes create functions each other can then recognize, so they run to a fixpoint (`-Dc28x.reg.rounds`, default 4) — running them once under-reports by ~4.5 points on a CPU2 image. Idempotent once converged. Also roots interrupt handlers from the PIE vector table's flash initializer and names them from `data/device_profiles/` (`FUN_00095de5` → `CANB0_INT`); that pass runs even with no dispatch registry, since its evidence is in flash. See [data/device_profiles/README.md](data/device_profiles/README.md).
...
- `SyncClaLabels.java` (either program, both in one project) — the two programs share the device's address map but Ghidra has no cross-program references, so this carries applied names and data types across in both directions. `SourceType` decides precedence (`DEFAULT < ANALYSIS < IMPORTED < USER_DEFINED`), so `SetupF28377D`'s precise register names replace `SetupClaProgram`'s coarse region labels, and equal-ranked disagreements are reported rather than resolved. Measured: **1821 labels + 196 data types**, 0 conflicts; a second run copies 0.
