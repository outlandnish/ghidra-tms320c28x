...
4d. `MarkComponentRegistry.java` — the other half of materialization. Materializing restores the *bytes* of the RAM dispatch table; this emits the *references*, so the call graph finally shows the indirect edges. Finds the table structurally, reads each dispatcher's descriptor field offset out of the instruction stream (the immediate before the `LCR *XARn`), creates functions at proven call targets, and recovers a dispatcher's own function when nothing calls it either. Idempotent. It also roots interrupt handlers from the PIE vector table's flash initializer and names them from `data/device_profiles/` (`FUN_00095de5` → `CANB0_INT`) — that pass runs even when no dispatch registry is found, since its evidence is in flash. See [data/device_profiles/README.md](data/device_profiles/README.md) for how a profile is generated and for the alignment check that stops it naming anything when the table and the profile disagree.
