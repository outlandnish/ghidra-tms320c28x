...
# to fail this pattern, so EVERY repeated instruction was silently missing from the TI side and
...
printf '%s\n' "$dis" | awk -v start="$start" '
  match($0, /^[[:space:]]*([0-9a-fA-F]{8})[[:space:]]+[0-9a-fA-F]{4}[[:space:]]+(\|\|)?[[:space:]]*([A-Z][A-Z0-9_]*)[[:space:]]*(.*)$/, m) {
